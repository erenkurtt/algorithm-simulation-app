...
function startKruskalGraph() {
  ...
  for (let j = 0; j < nodesCoordinates.length; j++) {
    if (
      nodesCoordinates[j][0] === "Y" ||
      nodesCoordinates[j][0] === "X" ||
      nodesCoordinates[j][0] === "W" ||
      nodesCoordinates[j][0] === "U"
    ) {
      let newObj = new componenttext(
        nodesCoordinates[j][0],
        nodesCoordinates[j][1] - 30,
        nodesCoordinates[j][2] + 30
      );
...
function updateCanvasKruskalGraph() {
  ...
  if (i < kruskalArray.length) {
    if (
      usedNodes.includes(kruskalArray[i][0]) === false ||
      usedNodes.includes(kruskalArray[i][1]) === false
    ) {
